...
const SecurityModal = ({
  ...
  onFinishSecurity,
  loadingSecurity,
  timeCounter // số giây muốn đếm (ví dụ 60)
}) => {
  ...
  // Text hiển thị warning (dùng i18n keys cũ)

  // Start countdown (an toàn: clear interval cũ nếu có)
    ...
    intervalRef.current = setInterval(() => {
      setSecondsLeft(prev => {
        if (prev <= 1) {
          ...
          setIsCounting(false);
          setErrorMessage('');
          // không resetFields ở đây nếu bạn muốn giữ value; nếu muốn clear, uncomment:
          // form.resetFields();
          return 0;
        }
        // cập nhật warning text mỗi giây
  ...
  // Khi user submit form: gọi callback parent và start countdown
  const handleFinish = (values) => {
    // Gọi callback bên ngoài (có thể là async). Mình vẫn start countdown ngay lập tức.
    try {
      onFinishSecurity && onFinishSecurity(values);
    } catch (err) {
      // nếu onFinishSecurity ném lỗi đồng bộ, vẫn start countdown theo yêu cầu.
      console.error(err);
    }
    // Bắt đầu countdown với timeCounter (nếu không có -> default 60s)
    startCountdown(timeCounter || 60);
    // reset field (xóa giá trị đã nhập ngay khi submit)
    form.resetFields();
  };
  ...
  useEffect(() => {
    ...
    // không return cleanup ở đây vì muốn giữ behavior khi modal vẫn mở
  }, [openSecurityModal]);
  ...
  // Handler khi người dùng đóng modal bằng nút/call
  const handleCancel = () => {
    if (intervalRef.current) {
      clearInterval(intervalRef.current);
      intervalRef.current = null;
    ...
    setErrorMessage('');
    form.resetFields();
    onCancelSecurityModal && onCancelSecurityModal();
  };
      ...
      >
        <div>
          ...
          >
            ...
            >
              <Input
                type='text' // dùng text để tránh một số vấn đề với type=number (mobile/format)
                inputMode="numeric"
                pattern="[0-9]*"
                onKeyPress={(e) => {
                ...
                maxLength={8}
                disabled={isCounting} // disabled khi đang đếm
              />
            </Form.Item>
            ...
            <Form.Item>
              <Button
                ...
                disabled={isCounting || loadingSecurity}
              >
              </Button>
